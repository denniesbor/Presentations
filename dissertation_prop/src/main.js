...
import cornerUrl from "../../shared/branding/upper_tri_or_half_sq_yellow.png";
import doubleCUrl from "../../shared/branding/double_c_yellow.png";
import yellowBarUrl from "../../shared/branding/yellow_parellelo.png";
import ribbonImgUrl from "../../shared/branding/gmu-ribbon.png";
...
function loadAssets() {
  ...
  const cornerNodes = document.querySelectorAll(
    'img[data-asset="corner-accent"]'
  );
  ...
  const yellowBarNodes = document.querySelectorAll(
    'img[data-asset="yellow-bar"]'
  );
  ...
  const powerMethodImg = document.querySelectorAll(
    'img[data-fig="power-method"]'
  );
  powerMethodImg.forEach((node) => {
    if (node) node.src = powerMethodUrl;
  });

  const powerNetworkImg = document.querySelectorAll(
    'img[data-fig="power-network"]'
  );
  powerNetworkImg.forEach((node) => {
    if (node) node.src = powerNetworkUrl;
  });

  const powerGicScenariosImg = document.querySelectorAll(
    'img[data-fig="power-gic-scenarios"]'
  );
  powerGicScenariosImg.forEach((node) => {
    if (node) node.src = powerGicScenariosUrl;
  });

  const powerFailuresImg = document.querySelectorAll(
    'img[data-fig="power-failures"]'
  );
  powerFailuresImg.forEach((node) => {
    if (node) node.src = powerFailuresUrl;
  });

  const powerEconomicImg = document.querySelectorAll(
    'img[data-fig="power-economic"]'
  );
  powerEconomicImg.forEach((node) => {
    if (node) node.src = powerEconomicUrl;
  });

  const gicValidationImg = document.querySelectorAll(
    'img[data-fig="gic-validation"]'
  );
  gicValidationImg.forEach((node) => {
    if (node) node.src = gicValidationRegUrl;
  });

  const secsValidationImg = document.querySelectorAll(
    'img[data-fig="secs-validation"]'
  );
  secsValidationImg.forEach((node) => {
    if (node) node.src = secsValidationUrl;
  });

  const gicValidationCompImg = document.querySelectorAll(
    'img[data-fig="gic-validation"]'
  );
  ...
  const powerFieldsHistoricalImg = document.querySelectorAll(
    'img[data-fig="power-fields-historical"]'
  );
  powerFieldsHistoricalImg.forEach((node) => {
    if (node) node.src = powerFieldsHistoricalUrl;
  });

  const powerFieldsExtremeImg = document.querySelectorAll(
    'img[data-fig="power-fields-extreme"]'
  );
  ...
  // Satellite figures
  const satFrameworkImg = document.querySelectorAll(
    'img[data-fig="sat-framework"]'
  );
  satFrameworkImg.forEach((node) => {
    if (node) node.src = satFrameworkUrl;
  });

  const satDistOrbImg = document.querySelectorAll(
    'img[data-fig="sat-dist-orb"]'
  );
  ...
  const satDensityImg = document.querySelectorAll(
    'img[data-fig="sat-density"]'
  );
  satDensityImg.forEach((node) => {
    if (node) node.src = satDensityUrl;
  });

  const satPropellantImg = document.querySelectorAll(
    'img[data-fig="sat-propellant"]'
  );
  ...
  const goesComparisonImg = document.querySelectorAll(
    'img[data-fig="goes-comparison"]'
  );
  ...
  const multiFrameworkImg = document.querySelectorAll(
    'img[data-fig="multi-framework"]'
  );
